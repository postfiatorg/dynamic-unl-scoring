...
| Short model name | `qwen3-next-80b-instruct` |
| Modal app name | `dynamic-unl-scoring` |
| Endpoint class | `ScoringEndpoint` |
| GPU | `H200` |
| Tensor parallelism | `1` |
| SGLang image | `lmsysorg/sglang:v0.5.6.post2-cu129-amd64-runtime` |
| Quantization | `fp8` |
...
| Modal volume | `scoring-model-weights` |
...
1. A Modal account.
2. Billing enabled in Modal. The endpoint uses an H200 GPU, so the first deploy
...
3. Downloads `Qwen/Qwen3-Next-80B-A3B-Instruct-FP8` into the
   `scoring-model-weights` Modal volume.
4. Pre-compiles DeepGEMM kernels on H200 during image build.
5. Creates a persistent Modal deployment named `dynamic-unl-scoring`.
...
in the Modal dashboard for the deployed `dynamic-unl-scoring` app.
...
```text
https://<workspace>--dynamic-unl-scoring-scoringendpoint-serve.modal.run/v1
```
...
```bash
modal app dashboard dynamic-unl-scoring
```
...
```bash
export MODAL_OPENAI_BASE_URL="https://<workspace>--dynamic-unl-scoring-scoringendpoint-serve.modal.run/v1"
```
...
The first request after idle can take several minutes because the H200 container
...
`scripts/score_validators.py` writes results under `results/modal/`. Do not run
...
5. Uses the `scoring-model-weights` Modal volume for the Hugging Face cache.
6. Downloads the selected Qwen3-Next model into that volume.
7. Runs `sglang.compile_deep_gemm` on H200 during image build.
8. Starts `python -m sglang.launch_server` with the selected model.
9. Enables deterministic inference and SGLang metrics.
