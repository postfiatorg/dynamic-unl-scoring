...
PROMPT_VERSION_CHOICES = ("v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8")
...
def build_scoring_v6_layer() -> dict[str, Any]:
    ...
    messages, identity_map = PromptBuilder(prompt_path=PROMPT_V6_PATH).build(
        scoring_snapshot
    )
    validator_id_map = {
        validator_id: identity["master_key"]
...
def build_scoring_v7_layer() -> dict[str, Any]:
    ...
    messages, identity_map = PromptBuilder(prompt_path=PROMPT_V7_PATH).build(
        scoring_snapshot
    )
    validator_id_map = {
        validator_id: identity["master_key"]
...
def build_scoring_v8_layer() -> dict[str, Any]:
    ...
    messages, identity_map = PromptBuilder(prompt_path=PROMPT_V8_PATH).build(
        scoring_snapshot
    )
    validator_id_map = {
        validator_id: identity["master_key"]
