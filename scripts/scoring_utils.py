...
PROMPT_V6_PATH = REPO_ROOT / "prompts" / "scoring_v6.txt"
PROMPT_V7_PATH = REPO_ROOT / "prompts" / "scoring_v7.txt"
...
PROMPT_VERSION_CHOICES = ("v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9")
...
def build_scoring_v2_layer() -> dict[str, Any]:
    ...
    messages, identity_map = PromptBuilder(prompt_path=PROMPT_V2_PATH, hidden_fields=set()).build(
        scoring_snapshot
    )
    validator_id_map = {
...
def build_scoring_v3_layer() -> dict[str, Any]:
    ...
    messages, identity_map = PromptBuilder(prompt_path=PROMPT_V3_PATH, hidden_fields=set()).build(
        scoring_snapshot
    )
    validator_id_map = {
        validator_id: identity["master_key"]
...
def build_scoring_v4_layer() -> dict[str, Any]:
    ...
    messages, identity_map = PromptBuilder(prompt_path=PROMPT_V4_PATH, hidden_fields=set()).build(
        scoring_snapshot
    )
    validator_id_map = {
        validator_id: identity["master_key"]
...
def build_scoring_v5_layer() -> dict[str, Any]:
    ...
    messages, identity_map = PromptBuilder(prompt_path=PROMPT_V5_PATH, hidden_fields=set()).build(
        scoring_snapshot
    )
    validator_id_map = {
        validator_id: identity["master_key"]
...
def build_scoring_v6_layer() -> dict[str, Any]:
    snapshot = load_snapshot()
    validators = [
        legacy_validator_to_profile(validator) for validator in snapshot["validators"]
    ]
    ...
    messages, identity_map = PromptBuilder(prompt_path=PROMPT_V6_PATH, hidden_fields=set()).build(
        scoring_snapshot
    )
    validator_id_map = {
        validator_id: identity["master_key"]
...
def build_scoring_v7_layer() -> dict[str, Any]:
    snapshot = load_snapshot()
    validators = [
        legacy_validator_to_profile(validator) for validator in snapshot["validators"]
    ]
    ...
    messages, identity_map = PromptBuilder(prompt_path=PROMPT_V7_PATH, hidden_fields=set()).build(
        scoring_snapshot
    )
    validator_id_map = {
        validator_id: identity["master_key"]
...
def build_scoring_v8_layer() -> dict[str, Any]:
    snapshot = load_snapshot()
    validators = [
        legacy_validator_to_profile(validator) for validator in snapshot["validators"]
    ]
    ...
    messages, identity_map = PromptBuilder(prompt_path=PROMPT_V8_PATH, hidden_fields=set()).build(
        scoring_snapshot
    )
    validator_id_map = {
        validator_id: identity["master_key"]
...
def build_prompt_layer(prompt_version: str) -> dict[str, Any]:
    ...
    if prompt_version == "v6":
        return build_scoring_v6_layer()
    if prompt_version == "v7":
        return build_scoring_v7_layer()
    if prompt_version == "v8":
        return build_scoring_v8_layer()
